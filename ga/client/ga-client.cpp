...
#if SDL_VERSION_ATLEAST(2,0,0)
#ifdef	ANDROID
	int wflag = SDL_WINDOW_FULLSCREEN | SDL_WINDOW_BORDERLESS;
#else
	int wflag = 0;
#endif
...
#define	DEBUG_FINGER(etf)	\
	...
	case SDL_FINGERDOWN:
		...
		sdlmsg_mousemotion(&m, mapx, mapy, 0);
		ctrl_client_sendmsg(&m, sizeof(m));
		//
		sdlmsg_mousekey(&m, 1, SDL_BUTTON_LEFT);
		ctrl_client_sendmsg(&m, sizeof(m));
		}
		break;
	case SDL_FINGERUP:
		...
		ctrl_client_sendmsg(&m, sizeof(m));
		//
		sdlmsg_mousekey(&m, 0, SDL_BUTTON_LEFT);
		ctrl_client_sendmsg(&m, sizeof(m));
		}
		break;
...
main(int argc, char *argv[]) {
	...
	char cwd[1024];
	myargv[0] = argv[0];
	myargv[1] = strdup("/sdcard/ga/rtsp.conf");
	myargv[2] = strdup("rtsp://140.121.150.93:8554/desktop");
	myargv[3] = NULL;
	argc = 3;
	argv = myargv;
	rtsperror("Launched from Android: pid=%d cwd=%s\n",
		getpid(), getcwd(cwd, sizeof(cwd)));
#endif
