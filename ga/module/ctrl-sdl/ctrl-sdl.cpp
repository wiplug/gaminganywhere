...
struct sdlmsg *
#if SDL_VERSION_ATLEAST(2,0,0)
sdlmsg_keyboard(struct sdlmsg *msg, unsigned char pressed, unsigned short scancode, SDL_Keycode key, unsigned short mod)
#else
sdlmsg_keyboard(struct sdlmsg *msg, unsigned char pressed, unsigned char scancode, SDLKey key, SDLMod mod)
#endif
{
...
struct sdlmsg *
sdlmsg_mousemotion(struct sdlmsg *msg, unsigned short mousex, unsigned short mousey, int relativeMouseMode) {
	//ga_error("sdl client: motion event x=%u y=%u\n", mousex, mousey);
	bzero(msg, sizeof(struct sdlmsg));
	msg->msgsize = htons(sizeof(struct sdlmsg));
...
sdlmsg_replay_init(void *arg) {
...
#elif defined __APPLE__
	ga_error("sdl replayer: Replay using CGPostEvent()\n");
#elif defined ANDROID
	...
	ga_error("sdl replayer: Replay using XTest (version %d.%d) for display %s screen %d.\n",
...
sdlmsg_replay_native(struct sdlmsg *msg) {
	...
	case SDL_EVENT_MSGTYPE_MOUSEMOTION:
		...
		if(msg->relativeMouseMode == 0) {
			...
			in.mi.dx = (short) msg->mousex;
			in.mi.dy = (short) msg->mousey;
...
sdlmsg_replay_native(struct sdlmsg *msg) {
	...
	case SDL_EVENT_MSGTYPE_MOUSEMOTION:
		...
		if(prect == NULL) {
			...
			pt.x = prect->left + msg->mousex;
			pt.y = prect->top + msg->mousey;
		}
	...
	case SDL_EVENT_MSGTYPE_MOUSEMOTION:
		...
		if(prect == NULL) {
			XTestFakeMotionEvent(display, screenNumber,
				msg->mousex, msg->mousey, CurrentTime);
		} else {
			XTestFakeMotionEvent(display, screenNumber,
				prect->left + msg->mousex,
				prect->top + msg->mousey, CurrentTime);
		}
		XSync(display, True);
		XTestGrabControl(display, False);
		break;
