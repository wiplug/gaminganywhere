...
#if 0
int
ga_win32_D3D_capture_YUV(struct SwsContext *swsctx, char *buf, int buflen, int *linesize) {
	...
	// convert to YUV
	src[0] = (unsigned char*) lockedRect.pBits;
	src[1] = NULL;
	srcstride[0] = lockedRect.Pitch;
	srcstride[1] = 0;
	dst[0] = (unsigned char *) buf;
	dst[1] = (unsigned char *) buf + screenRect.bottom * screenRect.right;
	dst[2] = (unsigned char *) buf + screenRect.bottom * screenRect.right + (screenRect.bottom * screenRect.right >> 2);
	dst[3] = NULL;
	linesize[0] = dststride[0] = screenRect.right;
	linesize[1] = dststride[1] = screenRect.right>>1;
	linesize[2] = dststride[2] = screenRect.right>>1;
	linesize[3] = dststride[3] = 0;
	sws_scale(swsctx, src, srcstride, 0, screenRect.bottom, dst, linesize);
	// Unlock
	g_pSurface->UnlockRect();
